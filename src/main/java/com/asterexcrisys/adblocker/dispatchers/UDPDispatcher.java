...
public final class UDPDispatcher implements Dispatcher {
    ...
    public UDPDispatcher(ExecutorService executor, EvaluationManager evaluationManager, ResolutionManager resolutionManager, BlockingQueue<UDPPacket> udpRequests, BlockingQueue<UDPPacket> udpResponses, List<Future<?>> udpHandlers, int requestsLimit, int minimumTasks, int maximumTasks) {
        Supplier<Runnable> task = () -> new UDPHandler(evaluationManager, resolutionManager, udpRequests, udpResponses);

...
public final class TCPDispatcher implements Dispatcher {
    ...
    public TCPDispatcher(ExecutorService executor, EvaluationManager evaluationManager, ResolutionManager resolutionManager, BlockingQueue<TCPPacket> tcpRequests, BlockingQueue<TCPPacket> tcpResponses, List<Future<?>> tcpHandlers, int requestsLimit, int minimumTasks, int maximumTasks, boolean isSecure) {
        Supplier<Runnable> task = () -> new TCPHandler(evaluationManager, resolutionManager, tcpRequests, tcpResponses);

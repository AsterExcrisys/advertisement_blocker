...
public final class TCPDispatcher implements Dispatcher {
    ...
    public TCPDispatcher(ExecutorService executor, BlockingQueue<TCPPacket> tcpRequests, BlockingQueue<TCPPacket> tcpResponses, List<Future<?>> tcpHandlers, ContextPool<ProxyManager> contextPool, int requestsLimit, int minimumTasks, int maximumTasks, boolean isSecure) {
        Supplier<Runnable> task = () -> new TCPHandler(contextPool, tcpRequests, tcpResponses);
